        ...
        <div className="bg-white border border-slate-300 rounded-lg p-4 shadow-xs space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-slate-200">
                ...
                <div className="flex items-center gap-2">
                    <Badge
                        variant={isDeadStock ? 'danger' : node.rotationStatus === 'FAST' ? 'success' : 'brand'}
                    >
                        {node.rotationStatus}
                    </Badge>
                    <Button
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="bg-slate-50 border border-slate-200 rounded p-2">
                    <span className="text-slate-500 text-[10px] uppercase font-mono block">Frozen Capital</span>
                    <span className="font-bold font-mono text-slate-900 text-sm">{formatCurrency(node.value)}</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded p-2">
                    <span className="text-slate-500 text-[10px] uppercase font-mono block">Units on Hand</span>
                    <span className="font-bold font-mono text-slate-900 text-sm">{formatNumber(node.quantityOnHand)} pcs</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded p-2">
                    <span className="text-slate-500 text-[10px] uppercase font-mono block">Unit Purchase Price</span>
                    <span className="font-bold font-mono text-slate-900 text-sm">{formatCurrency(node.unitPrice)}</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded p-2">
                    <span className="text-slate-500 text-[10px] uppercase font-mono block">Shelf Age (Turnover)</span>
